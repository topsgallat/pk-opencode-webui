...
export class GeminiProvider implements QuotaProvider {
  ...
  async isAvailable(options?: QuotaFetchOptions): Promise<boolean> {
    return Boolean(options?.resolveProviderAuthHeader?.(this.id) || resolveQuotaAuthHeader(options, GEMINI_QUOTA_URL))
  }

  async fetch(options: QuotaFetchOptions): Promise<QuotaProviderView> {
    try {
      const auth = options?.resolveProviderAuthHeader?.(this.id) || resolveQuotaAuthHeader(options, GEMINI_QUOTA_URL)
      if (!auth) {
        return {
          id: this.id,
          name: this.name,
